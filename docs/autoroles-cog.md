...
- **Adaptive request pacing:** starts at a ceiling of 60 requests per 10 minutes, halves that target after HTTP 429, and adds 10 requests back after every 100 successful responses.
- **Safety boundaries:** never exceeds 60 or falls below 20 requests per 10 minutes. This deliberately reserves shared API capacity for the interactive `/verify` command.
...
The request limiter initially spaces **every Habbo request** at least ten seconds apart: `600 seconds / 60 requests = 10 seconds per request`. This gives the maintenance cog a theoretical ceiling of **60 Habbo request starts per 10 minutes** across both the background updater and overlapping join-time syncs, leaving capacity available for `/verify`.

The pace adapts using a conservative decrease and gradual recovery strategy. A Habbo HTTP 429 immediately halves the current target (for example, 60 becomes 30 and the spacing becomes twenty seconds). Repeated 429 responses can lower it to the floor of 20 requests per 10 minutes. After every 100 successful HTTP 200 responses, the target rises by 10 until it reaches the 60-request ceiling again. A new 429 resets the success counter, so intermittent failures cannot cause a premature increase.

Each fully processed member needs **two Habbo requests**: one profile lookup and one groups lookup. The employee-motto safeguard reuses the first response rather than making a duplicate profile request. Consequently, 60 requests can fully check at most **30 members per 10 minutes** (`60 / 2 = 30`) when every lookup succeeds. Failed or partial profiles may use fewer requests, while API latency and Discord role writes can reduce completed throughput.

This is a local pacing ceiling, not a guarantee that Habbo will accept 60 requests from the shared public IP. Requests made by another bot are outside this cog's limiter and add to the IP's combined traffic. Any HTTP 429 still stops the current batch and activates the documented cooldown.
