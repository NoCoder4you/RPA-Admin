...
class ServerConfigStore:
    """Read audit channel configuration from serverconfig.json.

    Supported file shapes:
    1) {"audit_log_channel_id": "123"}
    2) {"guilds": {"<guild_id>": {"audit_log_channel_id": "123"}}}
    """
    ...
    def get_audit_channel_id(self, guild_id: int) -> int | None:
        """Return configured audit log channel ID for a guild, if configured."""
        ...
        if isinstance(guilds, dict):
            ...
            if isinstance(guild_entry, dict):
                channel_id = self._safe_int(guild_entry.get("audit_log_channel_id"))
                if channel_id is not None:
                    return channel_id

        # Fallback to a global channel setting for single-guild deployments.
