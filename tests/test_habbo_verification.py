...
class ServerConfigStoreTests(unittest.TestCase):
    """Validate audit-channel resolution from serverconfig.json."""

    def test_get_audit_channel_id_from_guild_specific_config(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            file_path = Path(temp_dir) / "serverconfig.json"
            file_path.write_text(
                json.dumps({"guilds": {"123": {"audit_log_channel_id": "456"}}}),
                encoding="utf-8",
            )

            store = ServerConfigStore(file_path=file_path)
            self.assertEqual(store.get_audit_channel_id(123), 456)

    def test_get_audit_channel_id_from_global_fallback(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            file_path = Path(temp_dir) / "serverconfig.json"
            file_path.write_text(json.dumps({"audit_log_channel_id": 999}), encoding="utf-8")

            store = ServerConfigStore(file_path=file_path)
            self.assertEqual(store.get_audit_channel_id(321), 999)
